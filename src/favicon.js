...
export const iconLink = (href, size, type = pngType) => ({
  rel: 'icon',
  type,
  href,
  sizes: `${size}x${size}`,
...
export const appleIconLink = (href, size) => ({
  rel: appleIconRel,
  href,
  sizes: `${size}x${size}`,
});

export const msSquare = (content, size) => ({
...
export default (
  ...
  if (s310x150)
    meta.push({ name: 'msapplication-wide310x150logo', content: s310x150 });
};
